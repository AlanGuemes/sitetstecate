...
export default function NormatividadClientPage({ documentos, enlacesExternos }: { documentos: any[], enlacesExternos: any[] }) {
  const categories = normatividadCategories.map(cat => {
    ...
    if (cat.id === "actas") return { ...cat, count: documentos.filter(d => d.subseccion?.nombre === "ACTAS DE SESION DE COMISION").length };
    return { ...cat, count: documentos.filter(d => d.category === cat.id).length };
  });
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && window.location.hash === "#sesionesdecabildo") {
      ...
      setTimeout(() => {
        const el = document.getElementById("sesionesdecabildo");
  ...
  const filteredDocuments = documentos.filter(doc => {
    const matchesCategory = selectedCategory === "all" ||
      (selectedCategory === "sesiones"
        ? doc.subseccion?.nombre === "SESIONES DE CABILDO"
        : selectedCategory === "actas"
          ? doc.subseccion?.nombre === "ACTAS DE SESION DE COMISION"
          : doc.category === selectedCategory)
    const searchLower = normalizeSearch(searchQuery)
    const matchesSearch = normalizeSearch(doc.title).includes(searchLower) ||
      (doc.description && normalizeSearch(doc.description).includes(searchLower)) ||
      (doc.ambito && normalizeSearch(doc.ambito).includes(searchLower))
  ...
  const generalDocs = filteredDocuments.filter(d => d.subseccion?.nombre !== "SESIONES DE CABILDO" && d.subseccion?.nombre !== "ACTAS DE SESION DE COMISION")
                    ...
                    <div className="space-y-3">
                      {enlacesExternos.map((enlace, index) => (
                        ...
                        >
                          ...
                          <div>
                            <p className="text-sm font-medium text-foreground">{enlace.titulo}</p>
                            <p className="text-xs text-muted-foreground">{enlace.descripcion}</p>
                          </div>
                        </a>
                      ))}
                    </div>
